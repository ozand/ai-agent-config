# TODO

## Bootstrap

- [x] Initialize the standalone local Git repository.
- [x] Add repository governance and secret-safety rules.
- [x] Add canonical model and routing policy.
- [x] Add sanitized LiteLLM endpoint profile.
- [x] Add Pi and OpenCode templates.
- [x] Add local credential creation documentation.
- [x] Add validation and regression tests.
- [x] Create and push the public remote repository `ozand/ai-agent-config`.
- [x] Add CI validation and rendered-template freshness checks.
- [x] Establish native issue governance in the new remote repository.
- [x] Record the next installer/merger increment as repository Issue #1.
- [x] Close bootstrap tracking Issue `ozand/servers_team#181` after final migration receipt verification.

## Future improvements

- [ ] Add a dry-run installer that merges client sections without overwriting unrelated configuration.
- [x] Add a renderer freshness check to CI after a remote repository exists.
- [ ] Add Linux/macOS secret-reader examples after validating them on target hosts.
